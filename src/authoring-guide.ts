...
const guideByTopic: Record<AuthoringTopic, string> = {
  workflow: "Snowstorm MCP workflow: query a few selected fields across close references; inspect only the best candidates; state the visual role; start with finite low-density emitters; dry-run batch edits; validate the complete package graph; probe numeric motion claims; render the relevant absolute-time window; then test in Minecraft. Give each layer one readable role: geometry, glow, motion, smoke, debris or accent.",
  ...
  magicspells: "MagicSpells guidance: helper custom-name must equal exactly one selector name; selector Scheme must match the particle file. Use a zero-velocity ParticleProjectileSpell for a short fixed layer and ArmorStandSpell for attached persistent effects. Keep delays in the parent MultiSpell. Use spell_timeline_inspect and spell_retime for timing-only changes. Do not add gameplay effects to a visual-only layer; test live timing in the target server.",
  validation: "Validation order: particle semantics and PNG/UV integrity, complete selector/helper/timeline graph, numeric Molang probes, deterministic first/middle/final scene evidence, then Minecraft playback. particle_verify_package validates all configured links and helper durations. A pass at one level does not prove the next; Snowstorm PNG/GIF/MP4 proves only the local simulator.",
